...
var form_id_principal;

function readyFn( jQuery ) {
    var form_class = document.querySelector('.form_name_main');
    if(form_class != null){
            form_id_principal = form_class.id;
    }else{
        form_id_principal = null;
    }
    console.log("id_formulario: "+form_id_principal);
}
...
function consumirApi(dep, prov, dist) {
     
    if(dep !== undefined && dep!== null && dep!== ""){ 
//      let concat_v= encodeURI('San Juan de Lurigancho'+', '+'Lima'); 
        mostrarCarga();                                           
        var concat_dep= encodeURI(dep); 
        var concat_prov= encodeURI(dep +','+prov); 
        var concat_dist= encodeURI(dep +','+prov+','+ dist); 
        var url_api  = 'https://nominatim.openstreetmap.org/search?format=json&limit=10&countrycodes=PE&q=';  
        var url_dist = url_api + concat_dist;  
        var url_prov = url_api + concat_prov;  
        var url_dep  = url_api + concat_dep;  

        fetch(url_dist ,{ mode: 'cors'})
        .then((resp) => resp.json())
        .then(function(data) { 
            //encontró distrito
            var res =data;
...
function bloquearBoton(id_form, id_boton, bloquear){
    console.log('bloquearBoton');
    if(form_id_principal != null || form_id_principal != undefined){
        if(id_form != "" && id_boton != ""  ){
...
//        if(form_id_principal != null || form_id_principal != undefined){
            bloquearBoton(form_id_principal, 'btnLPE_AgregarTipoContacto', true);
//        }
      }
...
//        if(form_id_principal != null || form_id_principal != undefined){
            bloquearBoton(form_id_principal, 'btnLPE_AgregarTipoContacto', false);
//        }
    }
}
...
function validarAlfanumerico_minimoLetras(event){
...
//            if(form_id_principal != null || form_id_principal != undefined){ 
                bloquearBoton(form_id_principal, 'btnLPE_AgregarSocio', true);
//            }
...
//            if(form_id_principal != null || form_id_principal != undefined){
                bloquearBoton(form_id_principal, 'btnLPE_AgregarSocio', false);
//            }
        }
    }
}
...
//            switch (event.value){
            var id_descrip = 'txtLPE_Alm_DescripMedioContacto';
            switch (event){
                    case "TP_MEDCO_COR"://"8": 
                            PrimeFaces.widgets['widget_'+form_id_principal+'_'+id_descrip].jq.attr("data-validar", "correo").attr("onkeypress", "validateAlphanumericCorreo(event)").attr("onblur", "validarCorreo(this)").attr("maxlength", "200");
...
//                            PrimeFaces.widgets.widget_FormRegSolicAuto_txtLPE_Alm_DescripMedioContacto.jq.attr("maxlength", "50");
                            break;
                    case "TP_MEDCO_FIJ"://9":
                            PrimeFaces.widgets['widget_'+form_id_principal+'_'+id_descrip].jq.removeAttr("onkeypress").removeAttr("onblur").attr("onkeypress","validateNumeros(event, 'fijo')").attr("maxlength", "20");
...
//                            PrimeFaces.widgets.widget_FormRegSolicAuto_txtLPE_Alm_DescripMedioContacto.jq.attr("maxlength", "20");
                            break;
                    case "TP_MEDCO_MOV"://10":  
                            PrimeFaces.widgets['widget_'+form_id_principal+'_'+id_descrip].jq.removeAttr("onkeypress").removeAttr("onblur").attr("onkeypress","validateNumeros(event, 'movil')").attr("maxlength", "20");
//                        PrimeFaces.widgets.widget_FormRegSolicAuto_txtLPE_Alm_DescripMedioContacto.jq.removeAttr("onkeypress");
//                        PrimeFaces.widgets.widget_FormRegSolicAuto_txtLPE_Alm_DescripMedioContacto.jq.removeAttr("onblur");
...
//                        PrimeFaces.widgets.widget_FormRegSolicAuto_txtLPE_Alm_DescripMedioContacto.jq.attr("maxlength", "20");
                    ...
                    case "TP_MEDCO_FAX"://"11":  
                            PrimeFaces.widgets['widget_'+form_id_principal+'_'+id_descrip].jq.removeAttr("onkeypress").removeAttr("onblur").attr("onkeypress","validateNumeros(event, 'fax')").attr("maxlength", "20");
//                        PrimeFaces.widgets.widget_FormRegSolicAuto_txtLPE_Alm_DescripMedioContacto.jq.removeAttr("onkeypress");
//                        PrimeFaces.widgets.widget_FormRegSolicAuto_txtLPE_Alm_DescripMedioContacto.jq.removeAttr("onblur");
...
//                        PrimeFaces.widgets.widget_FormRegSolicAuto_txtLPE_Alm_DescripMedioContacto.jq.attr("maxlength", "20");
                        break;
                    default:  
                            PrimeFaces.widgets['widget_'+form_id_principal+'_'+id_descrip].jq.removeAttr("onkeypress").removeAttr("onblur").attr("onkeypress","validateNumeros(event)");
//                        PrimeFaces.widgets.widget_FormRegSolicAuto_txtLPE_Alm_DescripMedioContacto.jq.removeAttr("onkeypress");
//                        PrimeFaces.widgets.widget_FormRegSolicAuto_txtLPE_Alm_DescripMedioContacto.jq.removeAttr("onblur");
//                        PrimeFaces.widgets.widget_FormRegSolicAuto_txtLPE_Alm_DescripMedioContacto.jq.attr("onkeypress","validateNumeros(event)"); 
            ...
            $("#error_msg_contacto").html("");
//            PrimeFaces.widgets.widget_FormRegSolicAuto_txtLPE_Alm_DescripMedioContacto.jq.removeAttr("onblur");
            PrimeFaces.widgets['widget_'+form_id_principal+'_'+id_descrip].jq.removeAttr("onblur");
       } 
   } 
