 ...
 */
@Stateless
public class GeppConsultaGuiasRenagiFacade {
    ...
    public GeppConsultaGuiasRenagiFacade() {
    }
    ...
    public List<HashMap> listarGuiasxFiltro(String tipo, String filtro) {
        String campo = "";
        if (tipo.equals("guia")) {
            campo = "AND RES2.NUMERO = ?1\n";
        }
        if (tipo.equals("ruc")) {
            campo = "AND PER1.RUC = ?1\n";
        }
        if (tipo.equals("expediente")) {
            campo = "AND REG.NRO_EXPEDIENTE = ?1\n";
        }
        if (tipo.equals("rg")) {
            campo = "AND RES.NUMERO = ?1\n";
        }
        Query q = em.createNativeQuery(
                "SELECT REG.ID \"PDF_ID\", PER1.RZN_SOCIAL||PER1.APE_PAT|| ' ' ||PER1.APE_MAT ||' ' ||PER1.NOMBRES AS RAZON_SOCIAL, PER1.RUC, TB.NOMBRE,\n"
                + "RES.NUMERO \"NRO_RG\", REG.NRO_EXPEDIENTE \"NRO_EXPEDIENTE\", RES2.NUMERO \"NRO_GT\", RES2.FECHA_INI, RES2.FECHA_FIN,\n"
