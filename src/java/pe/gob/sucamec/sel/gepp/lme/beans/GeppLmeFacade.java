 ...
 */
package pe.gob.sucamec.sel.gepp.lme.beans;
...
public class GeppLmeFacade {
    ...
    public List<HashMap> selectLike(LoginController usuario, String tipo, String filtro, boolean publico) {
        //TODO: REVISAR EN OTROS SISTEMAS NRO_EXPEDIENTE
        String sql = "SELECT L.ID, L.NRO_LICENCIA, E.RUC, E.RZN_SOCIAL, TB.NOMBRE AS TIPO_DOC, P.NUM_DOC, P.APE_PAT, P.APE_MAT, P.NOMBRES, R.NRO_EXPEDIENTE, "
                + "TC.NOMBRE AS TIPO_CARGO, FEC_EMI, FEC_VENC, TE.NOMBRE AS TIPO_EMISION, TL.NOMBRE AS TIPO_LICENCIA \n"
                + "FROM\n"
        ...
        if (tipo.equals("nroexp")) {
            sql += " AND R.NRO_EXPEDIENTE LIKE ?2 ";
        } else if (tipo.equals("nrolic")) {
            sql += " AND L.NRO_LICENCIA LIKE ?2 ";
        } else if (tipo.equals("numdoc")) {
            sql += " AND P.NUM_DOC LIKE ?2 ";
        }
        ...
        q.setParameter(2, nullATodo(filtro, false));
