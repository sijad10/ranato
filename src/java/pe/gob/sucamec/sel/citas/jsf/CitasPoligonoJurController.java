 ...
 * @author rarevalo
 */
@Named("citasPoligonoJurController")
@SessionScoped
public class CitasPoligonoJurController implements Serializable {
...
//    private String codVerificacion;
    ...
    private String nroDocVig;
    private String nomVigilante;
    private String docVigilante;
    ...
    private String mensajeNota1 = "";
    private String mensajeNota2 = "";
    ...
    // Solicitud Ventanilla virtual
    String eligeSolicitud;
    ...
    @EJB
    private pe.gob.sucamec.bdintegrado.bean.SbUsuarioFacadeGt ejbSbUsuario;
    //@EJB
    ...
    @EJB
    private pe.gob.sucamec.bdintegrado.bean.TipoBaseFacadeGt ejbTipoBaseGtFacade;
    @EJB
    private pe.gob.sucamec.bdintegrado.bean.TipoBaseFacade ejbTipoBaseRealFacade;
    ...
    public String getMensajeNota1() {
        return mensajeNota1;
    }

    public void setMensajeNota1(String mensajeNota1) {
        this.mensajeNota1 = mensajeNota1;
    }

    public String getMensajeNota2() {
        return mensajeNota2;
    }

    public void setMensajeNota2(String mensajeNota2) {
        this.mensajeNota2 = mensajeNota2;
    }
...
//        maxFecha = fecha.getTime();
    ...
    public Date getMaxFechaGeneral() {
        if (registro.getTipoSedeId() != null) {
            ...
            List<Map> lstconfiguracionSede = ejbFacade.listaConfiguraSede(ejbTipoBaseFacade.buscarTipoBaseXCodProg("TP_PROG_POL").getId(),registro.getTipoSedeId().getId(),ejbTipoBaseFacade.buscarTipoBaseXCodProg("TP_CIT_PRE").getId()); 
            for (Map item : lstconfiguracionSede) {
                ...
                if (item.get("NUM_DIAS_RESERVA") != null) {
            ...
            List<Date> lstFechaHabilitadaSede = ejbFacade.listaFechaHabilitadaSede(ejbTipoBaseFacade.buscarTipoBaseXCodProg("TP_PROG_POL").getId(),registro.getTipoSedeId().getId(),ejbTipoBaseFacade.buscarTipoBaseXCodProg("TP_CIT_PRE").getId()); 
            ...
            List<Map> lstSedeFechaHabilitadaFeriado = ejbFacade.listaSedeFechaHabilitadaFeriado(ejbTipoBaseFacade.buscarTipoBaseXCodProg("TP_PROG_POL").getId(),registro.getTipoSedeId().getId(),ejbTipoBaseFacade.buscarTipoBaseXCodProg("TP_CIT_PRE").getId(),fechaIni,fechaFin); 
            ...
            while (maxFechaCalcula.compareTo(JsfUtil.getFechaSinHora(fechaIniRango)) >= 0) {
                ...
                else
                {
                    ...
                    List<Map> lstSedeFechaHabilitadaFeriadoEspefifica = ejbFacade.listaSedeFechaHabilitadaFeriadoEspefifica(ejbTipoBaseFacade.buscarTipoBaseXCodProg("TP_PROG_POL").getId(),registro.getTipoSedeId().getId(),ejbTipoBaseFacade.buscarTipoBaseXCodProg("TP_CIT_PRE").getId(),fechaIniRango); 
    ...
    public String getNroDocVig() {
        return nroDocVig;
    }

    public void setNroDocVig(String nroDocVig) {
        this.nroDocVig = nroDocVig;
    }

    public String getNomVigilante() {
        return nomVigilante;
    }

    public void setNomVigilante(String nomVigilante) {
        this.nomVigilante = nomVigilante;
    }

    public String getDocVigilante() {
        return docVigilante;
    }

    public void setDocVigilante(String docVigilante) {
        this.docVigilante = docVigilante;
    }
    ...
    public String getEligeSolicitud() {
        return eligeSolicitud;
    }

    public void setEligeSolicitud(String eligeSolicitud) {
        this.eligeSolicitud = eligeSolicitud;
    }
                ...
                try {
                    ...
                    if (ejbPersonaFacade.buscarPersonaXNroDoc(perExamen.getNumDoc()).isEmpty()) {
                        if (perExamen.getTipoDoc().getCodProg().equals("TP_DOCID_DNI")) {
                            ...
                            CitaTurPersona turPerExamen = wsPideController.buscarReniec(perExamen);
                            if (turPerExamen != null) {
                                perExamen = turPerExamen;
                                turPerExamen = (CitaTurPersona) JsfUtil.entidadMayusculas(turPerExamen, "");
                                ejbPersonaFacade.create(turPerExamen);
                            } else {
                                perExamen.setActivo((short) 1);
                                perExamen.setAudLogin(JsfUtil.getLoggedUser().getLogin());
                    ...
                    switch (registro.getTipoTramiteId().getCodProg()) {
                        ...
                        case "TP_TRA_VP":
                            if (!ejbPersonaFacade.buscarPersonaXNroRuc(perPago.getRuc()).isEmpty()) {
                            ...
                            } else if (!ejbRma1369Facade.buscarEmpresaSeguridadXRuc(perPago.getRuc()).isEmpty()) {
                                Map mapaEmp = ejbRma1369Facade.buscarEmpresaSeguridadXRuc(perPago.getRuc()).get(0);
                                String rznSoc = "" + mapaEmp.get("empresa");
                                perPago.setRznSocial(rznSoc);
                                ...
                                //WEB SERVICE PARA CREAR EMPRESA DESDE SUNAT
                                perPago = wsPideController.buscarSunat(perPago.getRuc());
                                perPago = (CitaTurPersona) JsfUtil.entidadMayusculas(perPago, "");
                                ejbPersonaFacade.create(perPago);
                            }
                            break;
                        case "TP_TRA_SIS":
                            String rucSispe = perPago.getRuc();

                            if (!ejbPersonaFacade.buscarPersonaXNroRuc(rucSispe).isEmpty()) {
                                perPago = ejbPersonaFacade.buscarPersonaXNroRuc(rucSispe).get(0);
                            } else if (!ejbRma1369Facade.buscarPerJurSispeXRuc(rucSispe).isEmpty()) {
                                Map mapaSis = ejbRma1369Facade.buscarPerJurSispeXRuc(rucSispe).get(0);
                                String apeP = "" + mapaSis.get("apePat");
                                String apeM = "" + mapaSis.get("apePat");
                                String nom = "" + mapaSis.get("apePat");
                                perPago.setApePat(apeP);
                                perPago.setApeMat(apeM);
                                perPago.setNombres(nom);
                                perPago.setTipoId(ejbTipoBaseFacade.buscarTipoBaseXCodProg("TP_PER_JUR"));
                                perPago.setActivo((short) 1);
                                ejbPersonaFacade.create(perPago);
                            } else {
                                perExamen.setRuc(rucSispe);
...
//                    registro.setTipoSedeId(ejbTipoBaseFacade.buscarTipoBaseXCodProg("TP_AREA_TRAM"));
                    ...
                    if (solicitarRecibo || solicitarVoucherTemp) {
                        ...
                        if (solicitarVoucherTemp) {
                            ...
                            String path = ReportUtil.mostrarAnio(fecha) + File.separator + ReportUtil.mostrarMes(fecha) + File.separator;
                        ...
                        if (solicitarRecibo) {
                            newComprobante.setVoucherTasa(reciboBn.getImporte().toString());
                            newComprobante.setVoucherSeq(reciboBn.getNroSecuencia().toString());
                            newComprobante.setFechaPago(reciboBn.getFechaMovimiento());
                            newComprobante.setVoucherCta(reciboBn.getId().toString());
                    ...
                    for (CitaTurConstancia row : listaTipoLicencia) {
                        row.setTurnoId(registro);
                        row.setAudLogin(JsfUtil.getLoggedUser().getLogin());
                        row.setAudNumIp(JsfUtil.getNumIP());
                        row.setActivo((short) 1);
                        row.setComprobanteId(newComprobante);
                        row.setLicenciaId(licencia);
                    }
                    registro.setTurConstanciaList(new ArrayList());
                    registro.setTurConstanciaList(listaTipoLicencia);

                    /*
                    List<CitaTurConstancia> turconsLst = new ArrayList();

                    if (registro.getTipoTramiteId().getCodProg().equals("TP_TRA_MUL")) {
                        for (CitaTipoGamac mod : modalidadesSelected) {
                            CitaTurConstancia cons = new CitaTurConstancia();
                            cons.setTurnoId(registro);
                            cons.setModalidadId(mod);
                            cons.setAudLogin("WEB");
                            cons.setAudNumIp(JsfUtil.getNumIP());
                            cons.setActivo((short) 1);
                            cons = (CitaTurConstancia) JsfUtil.entidadMayusculas(cons, "");
                            turconsLst.add(cons);
                        }
                    } else if (registro.getTipoTramiteId().getCodProg().equals("TP_TRA_SIS")
                            || registro.getTipoTramiteId().getCodProg().equals("TP_TRA_VP")) {
                        CitaTurConstancia cons1 = new CitaTurConstancia();
                        cons1.setTurnoId(registro);
                        cons1.setModalidadId(registro.getTipoTramiteId());
                        cons1.setAudLogin("WEB");
                        cons1.setAudNumIp(JsfUtil.getNumIP());
                        cons1.setActivo((short) 1);
                        cons1 = (CitaTurConstancia) JsfUtil.entidadMayusculas(cons1, "");
                        turconsLst.add(cons1);

                        CitaTurConstancia cons2 = new CitaTurConstancia();
                        cons2.setTurnoId(registro);
                        cons2.setModalidadId(registro.getTipoTramiteId());
                        cons2.setAudLogin("WEB");
                        cons2.setAudNumIp(JsfUtil.getNumIP());
                        cons2.setActivo((short) 1);
                        cons2 = (CitaTurConstancia) JsfUtil.entidadMayusculas(cons2, "");
                        turconsLst.add(cons2);
                    } else {
                        CitaTurConstancia cons = new CitaTurConstancia();
                        cons.setTurnoId(registro);
                        cons.setModalidadId(registro.getTipoTramiteId());
                        cons.setAudLogin("WEB");
                        cons.setAudNumIp(JsfUtil.getNumIP());
                        cons.setActivo((short) 1);
                        cons = (CitaTurConstancia) JsfUtil.entidadMayusculas(cons, "");
                    ...
                    registro.setTurConstanciaList(new ArrayList());
                    registro.setTurConstanciaList(turconsLst);
                     */
                    //COMENTAR
...
//        }
        ...
        if (ejbSbUsuario.obtenerUsuarioXDocActivo(perExamen.getNumDoc()) == null) {
            flagValidar = false;
            JsfUtil.addErrorMessage("La persona con documento " + perExamen.getNumDoc() + " debe tener una cuenta SEL activa.  Los administrados involucrados en un procedimiento administrativo tramitado ante la SUCAMEC deben cumplir con dicho requerimiento según la DIRECTIVA Nº 21-2017-SUCAMEC");
        }
...
//                        flagValidar = false;
//                    }
//                    for (CitaTurTurno tt : tcBD.getTurTurnoList()) {
...
//                for (CitaTurTurno tt : ejbFacade.listarTurnosXPersonaYDia(p.getId(), JsfUtil.getFechaSinHora(registro.getFechaTurno()))) {
...
//            tipoTramite = ejbTipoGamacFacade.buscarTipoGamacXId(tipoTramite.getId());
            /*if (!registro.getTipoTramiteId().getCodProg().equals("TP_TRA_DEF")
                ...
                if (ejbRma1369Facade.buscarVigilanteEmpresaIntegrado(perPago.getRuc(), perExamen.getNumDoc(), registro.getTipoTramiteId().getCodProg()).isEmpty()) {
            ...
            List<Map> lstSedeTurnosCuposFechaHabilitada = ejbFacade.listaSedeTurnosCuposFechaHabilitada(ejbTipoBaseFacade.buscarTipoBaseXCodProg("TP_PROG_POL").getId(),registro.getTipoSedeId().getId(),ejbTipoBaseFacade.buscarTipoBaseXCodProg("TP_CIT_PRE").getId(),registro.getFechaTurno()); 
            ...
            if (solicitarRecibo) {
                if (eligeReciboLista) {
                    if (reciboBn == null) {
                        JsfUtil.addWarningMessage("Por favor ingresar el nro. de comprobante");
                        flagValidar = false;
                    } else if (!validarReciboElegido()) {
                        JsfUtil.addErrorMessage("El recibo ingresado no corresponde");
                        flagValidar = false;
                    }
                } else {
                    borrarValoresRecibo();
                    setDisableReciboBn(false);
                    JsfUtil.addWarningMessage("Ingresar el nro. de comprobante correctamente en el 2do paso");
                    flagValidar = false;
                }
...
//        visibleTipoTram = false;
        ...
        nroDocVig = nomVigilante = docVigilante = "";

        setOpeLicIni(false);
        setOpeLicRen(false);
        setEligeSolicitud(null);
        licencia = null;
        listaLicencia = new ArrayList<>();
        licenciaModalidad = "";
        ...
        perPago.setRuc(loginController.getUsuario().getPersona().getRuc());
        perPago.setRznSocial(loginController.getUsuario().getPersona().getRznSocial());
...
//        visibleCodVer = false;
     ...
     *
     */
    public CitasPoligonoJurController() {
//        restaurarProgramacion();
//        registro = new CitaTurTurno();
//        registro.setAudLogin("WEB");
...
//        turComprobante.setAudLogin("WEB");
...
//        perExamen = new TurPersona();
    ...
    public void inicializar() {
        ...
        turComprobante.setBancoId(ejbTipoBaseFacade.buscarTipoBaseXCodProg("TP_BCO_NAC"));
        turComprobante.setAudLogin(JsfUtil.getLoggedUser().getLogin());
        turComprobante.setAudNumIp(JsfUtil.getNumIP());
    ...
    public static class CitasPoligonoJurControllerConverter implements Converter {
        ...
        public Object getAsObject(FacesContext facesContext, UIComponent component, String value) {
            ...
            CitasPoligonoJurController c = (CitasPoligonoJurController) JsfUtil.obtenerBean("citasPoligonoJurController", CitasPoligonoJurController.class);
...
//        codVerificacion = null;
        ...
        if (fecha3.compareTo(fecha2) < 0) {
            ...
            JsfUtil.addErrorMessage("El turno seleccionado no es válido");
//                } else if (!mostrarSelector(registro.getProgramacionId())) {
//                    JsfUtil.addErrorMessage("El turno seleccionado no es válido");
...
//                    cargarTurnosDisponibles();
        ...
        //# VALIDAR FECHA PARA CITAS DE RENOVACIÓN #//
        Date maxReno = new GregorianCalendar(2019, Calendar.MAY, 6).getTime();

        if (maxReno.compareTo(fechaProcess) > 0) {
            lstTipOpe = ejbTipoGamacFacade.lstTipoOpePol();
        } else {
            lstTipOpe = ejbTipoGamacFacade.lstTipoOpePolRen();
        }
        //# #//
    }
    ...
    public void validaPaso3() {
        ...
        if (perExamen.getNumDoc() != null) {
            if (perExamen.getNumDoc().length() >= 8) {
                ...
                if (isOpeLicRen() && licencia == null) {
                    JsfUtil.addErrorMessage("Ingrese el número de documento del vigilante.");
                } else if (Objects.equals(registro.getTipoTramiteId().getCodProg(), "TP_TRA_MUL") && modalidadesSelected.size() < 2) {
                    JsfUtil.addErrorMessage("Al seleccionar el tipo de licencia Multimodal debe ingresar al menos 2 modalidades");
                    //                } else if (ejbSbRecibosFacade.listaRecibosXSecuencia(registro.getSecuenciaEmpoce(), registro.getMontoEmpoce(), registro.getFechaEmpoce()).isEmpty()) {
                    //                    JsfUtil.addErrorMessage("Recibo no encontrado; verificar el código, fecha y monto del vóucher");
                    //                } else if (!ejbSbReciboRegistroFacade.listaRecibosRegistroUso(registro.getSecuenciaEmpoce(), registro.getMontoEmpoce(), registro.getFechaEmpoce()).isEmpty()) {
                    //                    JsfUtil.addErrorMessage("El recibo ya está siendo usado por otro expediente");
                } else if (Objects.equals(registro.getTipoTramiteId().getCodProg(), "TP_TRA_VP") && !perPago.getRuc().startsWith("20") && (perPago.getRuc().length() != 11)) {
                    JsfUtil.addErrorMessage("Al seleccionar el tipo de licencia de VIGILANCIA PRIVADA el número de RUC debe empezar con 20");
                } else if (Objects.equals(registro.getTipoTramiteId().getCodProg(), "TP_TRA_SIS") && !(perPago.getRuc().startsWith("1")) && (perPago.getRuc().length() != 11)) {
                    JsfUtil.addErrorMessage("Al seleccionar el tipo de licencia de SISPE el número de RUC debe empezar con 10 ó 15");
                    /*} else if (sList.contains(registro.getTipoTramiteId().getCodProg()) && (ejbRma1369Facade.buscarVigilanteEmpresaFechaIntegrado(perPago.getRuc(), perExamen.getNumDoc(), registro.getTipoTramiteId().getCodProg(), registro.getFechaTurno()).isEmpty())) {
                    JsfUtil.addWarningMessage("El vigilante NO cuenta con carnet de identidad de seguridad privada vigente a la fecha de cita");*/
                } else if (!validarCertSalud(registro, perExamen)) {
                } else if (!validaVinculacionSolicitud()) { 
                } else if (validarVoucherTupa2018()) {
                    JsfUtil.addErrorMessage("El recibo ingresado ya está siendo usado en otra cita o en otro expediente");
                } else if (Objects.equals(registro.getTipoTramiteId().getCodProg(), "TP_TRA_VP") && !ejbAmaLicenciaDeUsoFacade.listarLicenciaSegVigXPerLicenciaXRenovar(perExamen.getNumDoc()).isEmpty()) {
                    JsfUtil.addErrorMessage("La persona ya cuenta con una licencia de VIGILANCIA PRIVADA vigente o vencida");
                } else if (validarArmasConstancia()) {
                    if (validarLicenciaExamen()) {
                        if (validarCitasPendientesxModalidad()) {
                            String horaProg = registro.getProgramacionId().getHora();
            ...
            JsfUtil.addErrorMessage("Debe elegir un vigilante!.");
        }
    ...
    public void calculaHoraPresentaTolerancia(String hora) {
        ...
        if (minutosAntesInicio == 0) {
            mensajeNota1 = "1. Debe presentarse a la hora de inicio.";
        } else {
            mensajeNota1 = "1. Debe presentarse " + minutosAntesInicio + " minutos antes de la hora de inicio.";
        }    
        
        if (minutosTolerancia == 0) {
            mensajeNota2 = "2. No se consideran minutos de tolerancia, caso contrario perderá su cita.";
        } else {
            mensajeNota2 = "2. Se considera " + minutosTolerancia + " minutos de tolerancia después de la hora de inicio, caso contrario perderá su cita.";
        }       
    }
    ...
    public boolean validarVoucherUso(Long secuencia, BigDecimal importe, Date fecha) {
        ...
        if (ejbFacade.listarTurnosXVoucherCta(secuencia, importe, fecha).isEmpty()) {
            return false;
        }
        CitaTurTurno tur = ejbFacade.listarTurnosXVoucherCta(secuencia, importe, fecha).get(0);
    ...
    public boolean validarCertSalud(CitaTurTurno reg, CitaTurPersona perExa) {
        ...
        if (!sList.contains(reg.getTipoTramiteId().getCodProg())) {
            SbCsCertifsalud certSalud = ejbSbCsCertifsaludFacade.obtenerCertificadosXSolicitanteGeneral(perExa.getNumDoc().trim());
            if (certSalud != null) {
                if (reg.getFechaTurno().compareTo(certSalud.getFechaVencimiento()) > 0)  {
                    JsfUtil.addErrorMessage("La persona no cuenta con un certificado de salud vigente (Fecha de Vencimiento " + JsfUtil.dateToString(certSalud.getFechaVencimiento(),"dd/MM/yyyy") + ") y está registrando la Fecha de la Cita el " + JsfUtil.dateToString(reg.getFechaTurno(),"dd/MM/yyyy"));
                    res = false;
                } else {
                    if (certSalud.getCondicionObtenidaId().getCodProg().equals("TP_COB_APT")) {
                        res = true;
                    } else {
                        JsfUtil.addErrorMessage("La persona cuenta con un certificado de salud con estado " + certSalud.getCondicionObtenidaId().getNombre());
                        res = false;
                    }
                }    
            } else {
                JsfUtil.addErrorMessage("La persona no cuenta con un certificado de salud");
                res = false;
            }
        } else {
            res = false;
    ...
    public boolean validarLicenciaExamen() {
        ...
        if (modalidadConLic && isOpeLicIni()) {
            JsfUtil.addErrorMessage("El vigilante ya cuenta con una licencia de uso para la modalidad seleccionada");
            paso = "paso3";
            tieneLicExa = true;
            // COMENTTAR | DESCOMENTAR PARA VALIDAR EXAMEN DE TIRO APROBADO
            /*} else if (modalidadAprobadaVig) {
                JsfUtil.addErrorMessage("El vigilante cuenta con un examen de tiro aprobado y vigente para la modalidad seleccionada");
                paso = "paso3";
                tieneLicExa = true;*/
        }
        ...
        if (fechaTurnoSeleccionada != null) {
            try {
                    ...
                    if (flag.equals("true")) {
                        ...
                        if (!sedesAfectoNoAtencion.contains(registro.getTipoSedeId().getCodProg())) {
                            listaProgramaciones = ejbTurProgramacionFacade.lstTurProgramacion(registro.getTipoSedeId().getId());
                        }
                    } else {
                        listaProgramaciones = ejbTurProgramacionFacade.lstTurProgramacion(registro.getTipoSedeId().getId());
                    }*/
                    ...
                    {    
                        List<Map> lstSedeTurnosCupos = ejbFacade.listaSedeTurnosCupos(ejbTipoBaseFacade.buscarTipoBaseXCodProg("TP_PROG_POL").getId(),registro.getTipoSedeId().getId(),ejbTipoBaseFacade.buscarTipoBaseXCodProg("TP_CIT_PRE").getId(),registro.getFechaTurno()); 
...
//                }
    ...
    public String mostrarProgramarTurno() {
        ...
        return "/aplicacion/citas/poligonoJur/ProgramarTurnoJur";
    }
...
//            visibleCodVer = false;
            ...
            turComprobante.setAudLogin(JsfUtil.getLoggedUser().getLogin());
            turComprobante.setAudLoginTurno(JsfUtil.getLoggedUser().getLogin());
            turComprobante.setAudNumIp(JsfUtil.getNumIP());
...
//        codVerificacion = null;
    ...
    public boolean validaCaptchaTC() {
        ...
        if (!terminos) {
            ...
            JsfUtil.invalidar("tabGestion:creaCitaPolJurForm:terminos");
            error = true;
        }
        // Validar Captcha //
        if (!captcha.ok()) {
            JsfUtil.addErrorMessage("Error el captcha es incorrecto");
            JsfUtil.invalidar("tabGestion:creaCitaPolJurForm:textoCaptcha");
            error = true;
        }
        if (error) {
...
//        }
    ...
    public List<String> autcompleteRuc(String query) {
        try {
            ...
            rucRznSocial = nroRuc = rznSocial = "";
            perPago = new CitaTurPersona();
            RequestContext.getCurrentInstance().update("tabGestion:creaCitaPolJurForm:rznSocial");
            return lstString;
    ...
    public List<String> autcompleteNroDoc(String query) {
        if (isOpeLicIni()) {
            try {
                List<ArrayRecord> list = ejbRma1369Facade.buscarVigilanteXEmpresaJurIntegrado(query, perPago.getRuc());
                List<String> lstString = new ArrayList<>();
                for (ArrayRecord row : list) {
                    String cadena = row.get("ID").toString();
                    lstString.add(cadena);
                }
                nroDocVig = docVigilante = "";
                nomVigilante = "";
                perExamen = new CitaTurPersona();
                RequestContext.getCurrentInstance().update("tabGestion:creaCitaPolJurForm:nomVigilante");
                return lstString;
    ...
    public void eligeVigilanteEmp(SelectEvent event) {
        if (event.getObject() != null) {
            docVigilante = (String) event.getObject().toString();
            String[] ids = docVigilante.split(",");
            nroDocVig = ids[0];
            nomVigilante = ids[1];
            String[] noms = nomVigilante.split("-");

            perExamen.setNumDoc(nroDocVig);
            perExamen.setApePat(noms[0]);
            perExamen.setApeMat(noms[1]);
            perExamen.setNombres(noms[2]);

            SbPersona per = ejbSbPersona.findByNumDoc(nroDocVig);
            try {
                //SbPersona per = ejbSbPersona.findByNumDoc(nroDocVig);
                perExamen.setNumDocVal(per.getNumDocVal());
                perExamen.setTipoDoc(ejbTipoBaseFacade.buscarTipoBaseXCodProg(per.getTipoDoc().getCodProg()));
                if (perExamen.getApePat().equals(" ")) {
                    perExamen.setApePat(per.getApePat());
                }
                if (perExamen.getApeMat().equals(" ")) {
                    perExamen.setApeMat(per.getApeMat());
                }
                if (perExamen.getNombres().equals(" ")) {
                    perExamen.setNombres(per.getNombres());
                }
            } catch (Exception e) {
            }
            ...
            if (isOpeLicIni()) {
                ...
                listaLicencia = ejbAmaLicenciaDeUsoFacade.listarLicenciaXPerLicenciaXRenovarJuridica(per.getId());
                if (!listaLicencia.isEmpty()) {
                    licencia = listaLicencia.get(0);
                    selectNroLicencia();
                }
            }
            //RequestContext.getCurrentInstance().update("tabGestion:creaCitaPolJurForm:nomVigilante");        
    ...
    public String obtenerNroDocVig(String value) {
        if (value != null) {
            try {
                String[] ids = value.split(",");
                nroDocVig = ids[0];
    ...
    public List<SbRecibos> autoCompleteRecibos(String query) {
        try {
            ...
            if (registro.getTipoTramiteId() != null && perExamen != null && perPago != null) {
                HashMap mMap = new HashMap();
                mMap.put("tipo", null);
                mMap.put("importe", parametrizacionTupa.getImporte());
                mMap.put("codigo", parametrizacionTupa.getCodTributo());
    ...
    public void lstnrSelectRecibo(SelectEvent event) {
        try {
            if (event != null) {
                SbExpVirtualSolicitud solExpVirtualUsado = reciboUsadoEnSolicitud((SbRecibos) event.getObject());
                if (solExpVirtualUsado != null) {
                    JsfUtil.mensajeError("Recibo " + reciboBn.getNroSecuencia() + " usado en una solicitud de Ventanilla Virtual del "
                            + solExpVirtualUsado.getProcedimientoTupaId().getTupaProcedimientoId().getAbreviatura()
                            + " registrado el " + JsfUtil.formatoFechaDdMmYyyy(solExpVirtualUsado.getFechaRegistro())
                            + ". Por favor seleccione la solicitud de la lista.");
                    JsfUtil.mensajeAdvertencia("Si NO DESEA PRESENTAR la solicitud por Ventanilla Virtual, eliminelo desde Listado de Solicitudes en Ventanilla Virtual.");
                    borrarValoresRecibo();
                } else {
                    borrarValoresRecibo();
    ...
    public void selectNroLicencia() {
        if (licencia != null) {
            ...
            if (sList.contains(licencia.getEstadoId().getCodProg())) {
                licenciaModalidad = "Nro Licencia: <b>" + licencia.getNroLicencia() + "</b> / ";
                ...
                for (AmaTipoLicencia tip : licencia.getAmaTipoLicenciaList()) {
                    if (tip.getModalidadId().getCodProg().equals("TP_MOD_SEG")) {
                        ...
                        licenciaModalidad += " <b>" + tip.getModalidadId().getNombre() + "</b>";
                        break;
                    }
    ...
    public List<SbExpVirtualSolicitud> listarSolicitudes() {
        SbPersona per = loginController.getUsuario().getPersona();//ejbSbPersona.findByNumDoc(loginController.getUsuario().getNumDoc().trim());
        String tupa = "";
        //System.out.println("registro.getTipoTramiteId()" + registro.getTipoTramiteId());
        if (registro.getTipoTramiteId() != null) {
            switch (registro.getTipoTramiteId().getCodProg()) {
                case "TP_TRA_DEF":
                    //tupa = "TUPA 72";
                    tupa = "TUPA 2022 - PA3400DED2";
                    break;
                case "TP_TRA_CAZ":
                    //tupa = "TUPA 76";
                    tupa = "TUPA 2022 - PA34007189";
                    break;
                case "TP_TRA_DEP":
                    //tupa = "TUPA 74";
                    tupa = "TUPA 2022 - PA3400A790";
                    break;
                case "TP_TRA_SIS":
                    //tupa = (isOpeLicRen()) ? "TUPA 81" : "TUPA 80";
                    tupa = (isOpeLicRen()) ? "TUPA 2022 - PA34004360" : "TUPA 2022 - PA340089AD";
                    break;
                case "TP_TRA_VP":
                    //tupa = (isOpeLicRen()) ? "TUPA 83" : "TUPA 82";
                    tupa = (isOpeLicRen()) ? "TUPA 2022 - PA3400C730" : "TUPA 2022 - PA3400C585";
                    break;
                case "TP_TRA_COL":
                    //tupa = "TUPA 88";
                    tupa = "TUPA 2022 - PA34004F01";
                    break;
            }
            return ejbSbExpVirtualSolicitudFacade.listarSolicitudesXCriterios(tupa, "procedTupa", per.getId(), "'TP_EXVIEST_REVTRA'", registro.getTipoSedeId().getId());
        }
        return null;
    }

    public void selectNroSolicitud() {
        borrarValoresRecibo();
        if (registro.getExpVirtualSolicitudId() != null) {
            reciboBn = registro.getExpVirtualSolicitudId().getComprobanteId();
            strNroSecuencia = "<b>" + reciboBn.getNroSecuencia() + "</b>";
    ...
    public void selectEligeSolicitud() {
        borrarValoresRecibo();
        registro.setExpVirtualSolicitudId(null);
        RequestContext context = RequestContext.getCurrentInstance();
        context.execute("$(\".contTipoLic, .contNroLic\").addClass(\"ui-helper-hidden\")");
        if (isOpeLicIni()) {
            context.execute("$(\".contTipoLic\").removeClass(\"ui-helper-hidden\")");
    ...
    public void vincularSolicitudCita() {
        if (registro.getExpVirtualSolicitudId() != null) {
            //actualizar estado a PRESENTADO
            registro.getExpVirtualSolicitudId().setEstadoId(ejbTipoBaseGtFacade.buscarTipoBaseXCodProg("TP_EXVIEST_LCP"));
            //registro.getExpVirtualSolicitudId().setFechaSolicitud(new Date());
            //registro.getExpVirtualSolicitudId().setNumeroSolicitud("SV" + JsfUtil.dateToString(registro.getExpVirtualSolicitudId().getFechaSolicitud(), "yyyy") + StringUtils.leftPad(ejbSbExpVirtualSolicitudFacade.obtenerNroSolicitud().toString(), 7, "0"));
            ejbSbExpVirtualSolicitudFacade.edit(registro.getExpVirtualSolicitudId());
        }
        return;
    }

    public String formatearFecha(Date fecha) {
        return (fecha != null) ? JsfUtil.formatoFechaDdMmYyyy(fecha) : "";
    }

    public SbExpVirtualSolicitud reciboUsadoEnSolicitud(SbRecibos recibo) {
        List<SbExpVirtualSolicitud> lst = listarSolicitudes();
        for (SbExpVirtualSolicitud r : lst) {
            if (Objects.equals(r.getComprobanteId(), recibo)) {
                return r;
            }
        }
        return null;
    ...
    public boolean validaVinculacionSolicitud() {
        if (eligeSolicitud == null) {
            JsfUtil.addErrorMessage("Debe seleccionar si presentará o no el trámite por Ventanilla Virtual.");
            return false;
        } else if (eligeSolicitud.equals("SI") && registro.getExpVirtualSolicitudId() == null) {
            JsfUtil.addErrorMessage("Debe seleccionar una solicitud de Ventanilla Virtual.");
            return false;
        }
        return true;
    }
    
    public String getListCentralDiasInHabilitados(){
        List<TipoBaseGt> ListCentralDiasInHabilitados = ejbTipoBaseRealFacade.lstTipoBaseInactivo("TP_CENTRAL_POLIG_DIA");
        String s = "[";
        for (TipoBaseGt CentralDiasInHabilitados : ListCentralDiasInHabilitados) {
            s = s + "" + CentralDiasInHabilitados.getAbreviatura() + ",";
        }
        s = s.substring(0, s.length() - 1);
        s += "]";
        return s;
    ...
    public String getListZonalDiasInHabilitados(){
        List<TipoBaseGt> ListZonalDiasInHabilitados = ejbTipoBaseRealFacade.lstTipoBaseInactivo("TP_ZONAL_POLIG_DIA");
        String s = "[";
        for (TipoBaseGt ZonalDiasInHabilitados : ListZonalDiasInHabilitados) {
            s = s + "" + ZonalDiasInHabilitados.getAbreviatura() + ",";
        }
        s = s.substring(0, s.length() - 1);
        s += "]";
        return s;
