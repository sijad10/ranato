 ...
 * @author rarevalo
 */
@Named("citasVerificacionController")
@SessionScoped
public class CitasVerificacionController implements Serializable {
...
//    private String codVerificacion;
     ...
     *
     */
    public CitasVerificacionController() {
            ...
            if (validarCreate()) {
                try {
                    ...
                    if (ejbPersonaFacade.buscarPersonaXNroDoc(perExamen.getNumDoc()).isEmpty()) {
                        ...
                        perExamen = (CitaTurPersona) JsfUtil.entidadMayusculas(perExamen, "");
                        //}
                        if (tipoSubTramite.getCodProg().equals("TP_PROG_VER_ANR") || tipoSubTramite.getCodProg().equals("TP_PROG_VER_TRA") || tipoSubTramite.getCodProg().equals("TP_PROG_VER_FAP") || tipoSubTramite.getCodProg().equals("TP_PROG_VER_TNJ") ) {
...
//                    registro.setTipoSedeId(ejbTipoBaseFacade.buscarTipoBaseXCodProg("TP_AREA_TRAM"));
                    ...
                    switch (tipoSubTramite.getCodProg()) {
                        case "TP_PROG_VER_RLU":
                            for (CitaDetalleArma det : lstDetalleArmaSelected) {
                                if (det.getParaLista() == 1) {
                                    ...
                                    licReg.setModelo(det.getModeloId());
                                    licReg.setNroLicenciaCaf(det.getNroLicenciaCaf());
                    ...
                    if (registro.getId() != null) {
                        ...
                        /* ACTUALIZAR PERSONA SI ES SUBTRAMITE DE POLICIA */
                        if (tipoSubTramite.getCodProg().equals("TP_PROG_VER_ANR")) {
                            ...
                            //ejbSbPersonaFacade.edit(perUser);
                        }
                        /* */
...
//        visibleCodVer = false;
    ...
    public void eligeSubTramite(SelectEvent event) {
        ...
        context.update("tabGestion:crearCitaVerForm:panelPaso1");
        context.update("tabGestion:crearCitaVerForm:pnlContFFAA");
        context.update("tabGestion:crearCitaVerForm:pnlContMsg");
        context.update("tabGestion:crearCitaVerForm:pnlContEmpoceRLU");
        context.update("tabGestion:crearCitaVerForm:armasDT");
        if (perUser != null) {
    ...
    public void eligeInstitucion(SelectEvent event) {
        ...
        //RequestContext.getCurrentInstance().update("tabGestion:crearCitaVerForm:panelPaso1");
        //RequestContext.getCurrentInstance().update("tabGestion:crearCitaVerForm:pnlContFFAA");
    }
...
//                }
    ...
    public void siguiente(String p) {
        switch (p) {
            case "paso12":
                validaPaso1();
                RequestContext.getCurrentInstance().update("tabGestion:crearCitaVerForm");
                break;
            case "paso23":
                validaPaso2();
                break;
...
//                    }
    ...
    public String prepareModulo() {
        ...
        //listaArmas();
        return "/aplicacion/citas/verificacion/ReservarCita";
    }
...
//      codVerificacion = null;
        ...
        if (tipoTramite != null) {
            ...
            if (tipoSubTramite == null) {
                ...
                /*} else {
                if (tipoSubTramite.getCodProg().equals("TP_PROG_VER_ANR")) {
                    if (loginController.getUsuario().getPersona().getNroLicenciaCaf()==null) {
                        JsfUtil.addErrorMessage("Usted está Registrado como persona Natural en el Sistema, comunicarse con la Gerencia de Armas para "
                                + "que lo registre como miembro de las Fuerzas Armadas o Policia Nacional");
                        faltaDato=true;
                    }                                
            ...
            if (lstDetalleArma.size() <= 0) {
                ...
                if (!enLista) {
                    JsfUtil.addErrorMessage("No hay armas para verificación!");
                    faltaDato = true;
                }
            }
            ...
            if (!faltaDato) {
                switch (tipoSubTramite.getCodProg()) {
                    case "TP_PROG_VER_RLU":
                        if (lstDetalleArmaSelected.size() <= 0) {
                            JsfUtil.addErrorMessage("Debe seleccionar por lo menos un arma!");
                            faltaDato = true;
                        }
                        break;
...
//        codVerificacion = null;
    ...
    public void loadBuscaArma() {
        ...
        if (mostarBusqueda) {
            RequestContext context = RequestContext.getCurrentInstance();
            context.update("tabGestion:buscarArmaForm");
            context.execute("PF('wvDlgBuscarArma').show();");
        }
    }
    ...
    public void seleccionarTarjeta(Map mapa) {
        ...
        if (!existeArma) {
            ...
            if (!validarVoucherTupa2018Paso1()) {
                ...
                //Cuando no tiene solicita recibo y debe registrar el Expediente y Oficio
                if (solicitarRecibo == false) {
                    //TP_PROG_VER_TNJ  TRANSFERENCIA A FAVOR DE PERSONA NATURAL,PERSONA JURÍDICA
                    if (tipoSubTramite.getCodProg().equals("TP_PROG_VER_TNJ")) {
                        if (nroExpOficio == null) {
                            JsfUtil.mensajeAdvertencia("Debe de Ingresar el Nro. de Expediente");
                            agregar = false;
                            return;
                        }    
                        if (nroOficioSelected == null) {
                            JsfUtil.mensajeAdvertencia("Debe de Seleccionar el Nro. de Oficio");
                            agregar = false;
                            return;
                        }   
                    }
                }
                //Fin de Cuando no tiene solicita recibo y debe registrar el Expediente y Oficio
    ...
    public void loadAddArma() { // hito 1 :revisar cuantos usan este metodo
        ...
        if (mostarAdd) {
            RequestContext context = RequestContext.getCurrentInstance();
            context.update("tabGestion:addArmaForm");
            context.execute("PF('wvDlgAddArma').show();");
        }
    }
            ...
            if (agregar) {
                ...
                /*if (encontrado){
                    if (nroLicDisca.equals("")) {
                        AmaArma armaGa = gestionCitasFacade.AmaArmaxSerie(detalleArma.getNroSerie().trim());
                        if (armaGa != null) {
                            detalleArma.setArmaId(armaGa);
                            detalleArma.setNroRua(armaGa.getNroRua());
                            detalleArma.setNroSerie(armaGa.getSerie());
                            detalleArma.setTipoArma(armaGa.getModeloId().getTipoArmaId().getNombre());
                            detalleArma.setMarca(armaGa.getModeloId().getMarcaId().getNombre());
                            detalleArma.setModelo(armaGa.getModeloId().getModelo());
                            detalleArma.setEstado(armaGa.getEstadoId().getNombre());
                            detalleArma.setCalibre(obtenerCalibreArma(armaGa.getModeloId().getAmaCatalogoList()));
                            detalleArma.setSituacion(armaGa.getSituacionId().getNombre());
                            detalleArma.setSituacionId(ejbTipoGamacFacade.buscarTipoGamacXCodProg(armaGa.getSituacionId().getCodProg()));
                            detalleArma.setParaLista(1);
                        }
                    }
                } else {*/
                ...
                RequestContext context = RequestContext.getCurrentInstance();
                context.execute("PF('wvDlgAddArma').hide();");
                context.update("tabGestion:crearCitaVerForm:panelPaso1");
                context.update("tabGestion:crearCitaVerForm:pnlContFFAA");
                context.update("tabGestion:crearCitaVerForm:armasDT");
                context.execute("selectPasos();");
