 ...
 * @author rarevalo
 */
@Named("citasPoligonoController")
@SessionScoped
public class CitasPoligonoController implements Serializable {
                ...
                try {
                    ...
                    if (ejbPersonaFacade.buscarPersonaXNroDoc(perExamen.getNumDoc()).isEmpty()) {
                        if (perExamen.getTipoDoc().getCodProg().equals("TP_DOCID_DNI")) {
                            ...
                            //perExamen = wsPideController.buscarReniec(perExamen);
...
//                    registro.setTipoSedeId(ejbTipoBaseFacade.buscarTipoBaseXCodProg("TP_AREA_TRAM"));
                    ...
                    if (registro.getTipoTramiteId().getCodProg().equals("TP_TRA_MUL")) {
                        for (CitaTipoGamac mod : modalidadesSelected) {
                            ...
                            cons.setAudLogin(loginController.getUsuario().getLogin());
                        ...
                        cons1.setAudLogin(loginController.getUsuario().getLogin());
                        ...
                        cons2.setAudLogin(loginController.getUsuario().getLogin());
...
//        visibleCodVer = false;
     ...
     *
     */
    public CitasPoligonoController() {
...
//        perExamen = new TurPersona();
    ...
    public static class TurTurnoControllerConverter implements Converter {
        ...
        public Object getAsObject(FacesContext facesContext, UIComponent component, String value) {
            ...
            CitasPoligonoController c = (CitasPoligonoController) JsfUtil.obtenerBean("citasPoligonoController", CitasPoligonoController.class);
...
//                    cargarTurnosDisponibles();
            ...
            if (perExamen.getNumDoc().length() >= 8) {
                ...
                paso = "paso3";
                // ACTIVAR PARA VALIDAR CITAS PENDIENTES
                /*if (validarCitasPendientes()) {
                    paso = "paso3";
                } else {
                    paso = "paso1";
                }*/
    ...
    public void validaPaso3() {
        ...
        if (sList.contains(registro.getTipoTramiteId().getCodProg()) && perPago.getRuc() == null) {
            JsfUtil.addErrorMessage("Ingrese y seleccione un número de RUC");
        } else if (Objects.equals(registro.getTipoTramiteId().getCodProg(), "TP_TRA_MUL") && modalidadesSelected.size() < 2) {
            JsfUtil.addErrorMessage("Debe seleccionar más de una modalidad");
            //JsfUtil.addErrorMessage("Al seleccionar el tipo de licencia Multimodal debe seleccionar más de una modalidad");
...
//                    JsfUtil.addErrorMessage("El recibo ya está siendo usado por otro expediente");
        ...
        } else if (sList.contains(registro.getTipoTramiteId().getCodProg()) && (ejbRma1369Facade.buscarVigilanteEmpresaFechaIntegrado(perPago.getRuc(), perExamen.getNumDoc(), registro.getTipoTramiteId().getCodProg(), registro.getFechaTurno(), false).isEmpty())) {
            JsfUtil.addWarningMessage("Usted no cuenta con carnet de identidad de seguridad privada a la fecha de cita");
        ...
        } else if (Objects.equals(registro.getTipoTramiteId().getCodProg(), "TP_TRA_VP")
                && registro.getSubTramiteId().getCodProg().equals("TP_TRAM_POL_INI")
                && !ejbAmaLicenciaDeUsoFacade.listarLicenciaSegVigXPerLicenciaXRenovar(perExamen.getNumDoc()).isEmpty()) {
            ...
            if (validarLicenciaExamen()) {
                if (validarCitasPendientesxModalidad()) {
                  ...
                  /*String horaProg = registro.getProgramacionId().getHora();
                    ...
                    //horaTolera = horaProg.substring(0, 4) + "5";                    
                    String minTolera = horaProg.substring(3,5);
                    minTolera = String.valueOf(Integer.parseInt(minTolera) + 5);
                    if(minTolera.length()==1){
                        horaTolera = horaProg.substring(0, 3) + "0" + minTolera;
                    }else {
                        horaTolera = horaProg.substring(0, 3) + minTolera;
                    }
...
//        codVerificacion = null;
    ...
    public void vincularSolicitudCita() {
        if (registro.getExpVirtualSolicitudId() != null) {
            //actualizar estado a CON CITA POLIGONO RESERVADA
