...
//        visibleRegDatos = true;
    ...
    public boolean mostrarSelector(CitaTurProgramacion item) {
        ...
        if (item != null) {
            if (item.getId() != null) {
                ...
                lstTurno = ejbFacade.listarTurnosXFechaTurno(registro.getFechaTurno(), item.getId());
    ...
    public String prepareModulo() {
        ...
        //listaArmas();
        return "/aplicacion/citas/ReservarCita";
    }

    public String mostrarReprogramarTurno() {
//        restaurarProgramacion();
        return "/aplicacion/turTurnoInterna/ReprogramarTurno";
    }

    public String mostrarConfirmarTurno() {
        restaurarProgramacion();
        return "/aplicacion/turTurnoInterna/ConfirmarTurno";
    }

    public String mostrarListadoTurnos() {
        restaurarProgramacion();
        return "/aplicacion/turTurnoInterna/ListarProgramacionTurno";
    }

    public String mostrarReporteTurnos() {
        restaurarProgramacion();
        return "/aplicacion/turTurnoInterna/ListarTurnoCC";
    }
...
//            visibleCodVer = false;
            ...
            turComprobante.setAudLogin("WEB");
            turComprobante.setAudLoginTurno("WEB");
            turComprobante.setAudNumIp(JsfUtil.getNumIP());
...
//        codVerificacion = null;
    ...
    public boolean validaCaptchaTC() {
        ...
        if (!terminos) {
            ...
            JsfUtil.invalidar(":crearForm:terminos");
            error = true;
        }
        // Validar Captcha //
        if (!captcha.ok()) {
            JsfUtil.addErrorMessage("Error el captcha es incorrecto");
            JsfUtil.invalidar(":crearForm:textoCaptcha");
            error = true;
        }
        if (error) {
...
//        }
    ...
    public void addArmaGrilla(){
        ...
        String nroSerie = JsfUtil.getRequestParameter("crearForm:nroSerie"); 
        String nroLic = JsfUtil.getRequestParameter("crearForm:nroLicencia"); 
        nroSerie = nroSerie.trim();
        nroLic = nroLic.replace(" ", "").trim();
                
        if(source.equals("crearForm:addArma")){
