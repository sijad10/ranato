...
@Named("dcGsspTotVigilantesController")
@SessionScoped
 ...
 */
public class DcGsspTotVigilantesController implements Serializable {
     ...
     */
    private ListDataModel<ArrayRecord> resultados = null;
    /**
     * Lista de resultados de la búsqueda
     */
    private ListDataModel<ArrayRecord> armas = null;
    /**
     * Lista de resultados de la búsqueda
     */
    private ListDataModel<ArrayRecord> historico = null;
    /**
     * Lista de resultados de la búsqueda
     */
    ListDataModel<ArrayRecord> cursos = null;
    ...
    public String direccionarBuscar() {
        resultados = null;
        mostrarBuscar();
        return "/aplicacion/consultas/DcGsspTotVigilantes/List";
    }
    ...
    public void obtenerNumCaract() {
        filtro = null;
        if (tipo != null) {
            if (Objects.equals(tipo, "dni")) {
                setCantNumeros("8");
            } else if (Objects.equals(tipo, "ce")) {
                setCantNumeros("9");
            } else {
                setCantNumeros("15");
            }
    ...
    public void buscar() {
        ...
        if (tipo != null) {
            if (!tipo.equals("carne")) {
                numdoc = filtro;
                if (numdoc.length() < Integer.valueOf(cantNumeros)) {
                    JsfUtil.mensajeAdvertencia("Debe ingresar " + cantNumeros + " dígitos.");
                    return;
                }
            } else if (tipo.equals("carne")) {
                carne = filtro;
            }
            resultados = new ListDataModel(obtenerListadoVigilantes(numdoc, carne));
        }
    }

    public List<Map> obtenerListadoVigilantes(String numdoc, String carne) {
        List<Map> listado = new ArrayList();

        listado = ejbSspCarneFacade.listarTotVigilantes(numdoc, carne);

        if (listado.isEmpty()) {
            listado = ejbSspCarneFacade.listarVigilantesHistorico(numdoc, carne);
        }

        if (listado.isEmpty()) {
            if ((numdoc != null && !numdoc.isEmpty())) {
                List<Map> listadoCursos = ejbSspRegistroCursoFacade.listarInfVigilantesSoloCursosIntegrado(numdoc);
                for (Map vig : listadoCursos) {
                    listado.add(vig);
                }
            }
        }
    ...
    public void mostrarVer() {
        ...
        if (registro.get("TIP_USR").toString().equals("5")) {
            codUsr = registro.get("CRNT_EXT").toString();
        } else {
            codUsr = registro.get("COD_USR").toString();
        }
        ...
        setHistorico((ListDataModel<ArrayRecord>) new ListDataModel(ejbSspCarneFacade.listarHistVigilantes(codUsr)));
        setCursos(null);
        List<Map> listCursos = ejbSspRegistroCursoFacade.listarCursosVigilantesIntegrado(codUsr);
        if(listCursos != null){
            setCursos((ListDataModel<ArrayRecord>) new ListDataModel(listCursos));
        }
        crearFoto(registro);
        estado = EstadoCrud.VER;
    }

    public void crearFoto(ArrayRecord item) {
        try {
            if (item != null) {
                String codUsr = (String) item.get("COD_USR"), nombreFoto = "";
                if(item.get("NOMBRE_FOTO") != null && !item.get("NOMBRE_FOTO").toString().equals("S/F")){
                    nombreFoto = item.get("NOMBRE_FOTO").toString();
                    ...
                    try {
                        ...
                        setFoto(JsfUtil.oracleTiffenJpg(fotoByte, 180, 240));
                    } catch (Exception e) {
                    ...
                    if (Objects.equals((String) item.get("DES_USR"), "EXTRANJERO")) {
                        codUsr = consultasDiscaFacade.obtenerCodUsrExtranjero(codUsr);
                    }
                    setFoto(JsfUtil.oracleTiffenJpg(consultasDiscaFacade.selectFoto(codUsr == null ? "" : codUsr.toUpperCase().trim(), true), 180, 240));
                }
            }
        } catch (Exception e) {
    ...
    public List<Map> obtenerListaCursos(ListDataModel<ArrayRecord> listIn) {
        ...
        if (listIn != null && listIn.getRowCount() > 0) {
            ...
            List<ArrayRecord> lstTemp = (List<ArrayRecord>) listIn.getWrappedData();
            for(Object temp: lstTemp){
                Map item = (Map) temp;
                curso = new HashMap();
     ...
     *
     */
    public DcGsspTotVigilantesController() {
        estado = EstadoCrud.BUSCAR;
    }
    ...
    public String obtenerCodUsr(Object reg) {
        String codigo = "";
        Map item = (Map) reg;
        if (item.get("TIP_USR").toString().equals("2")) {
            codigo = item.get("COD_USR").toString();
        } else {
            codigo = item.get("CRNT_EXT").toString();
        }

        return codigo;
    }

    public Boolean documentoValido(String valor) {
    ...
    public String obtenerEstadoVigilante(Map vig) {
        if (vig != null) {
            String estado = vig.get("ESTADO") == null ? "" : "" + vig.get("ESTADO");
    ...
    public String obtenerEstadoVigilanteView() {
        String estado = "" + registro.get("ESTADO");

        if (registro.get("TIPO_REG").equals("H")) {
            estado = "CESADO";
        }
        return estado;
    }
